...
$popupDetails = array();
$query = "SELECT eng_text, image, link_type, link_id, link_text_eng, eng_title, external_link FROM ".TBL_POPUP."  WHERE 1 ORDER BY id ASC LIMIT 1";
...
if($popupDetails['2']==""){ $popupDetails['2'] = ""; $popupDetails['3'] = ""; $popupDetails['4'] = ""; $popupDetails['6'] = ""; }
        ...
        if($popupDetails['2']=="business") {
            $query2 = " SELECT bd.name, bdi.business_logo,(select IF(cd.parent_id = 0,cd.category_id,cd.parent_id)) as parent_category_id, cd.title FROM ".TBL_BUSINESS." bd
                        ...
                        WHERE bd.deleted='0' AND bdi.lng_id = 1 AND cd.lng_id = 1 AND bd.id = ".$popupDetails['3']." AND bd.status = 'A'";
            //echo $query2; exit;
...
//            $popupDetails['popup_poll_name'] = $pollDetails['0'];
        ...
        elseif($popupDetails['2']=="category") {
            $query4 = "SELECT cm.name, cd.category_logo, cd.service1, cd.service2, cd.service3 FROM ".TBL_BLOG_CATEGORIES." cm LEFT JOIN  ".TBL_BLOG_CAT_DESC." cd ON cm.id = cd.category_id WHERE deleted='0' AND flag='1' AND cd.lng_id = 1 AND cm.id = ".$popupDetails['3']." "; 
            //echo $query4; exit;
