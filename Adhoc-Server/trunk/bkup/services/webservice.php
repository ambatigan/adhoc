...
$query = " INSERT INTO comments (
		...
		if(mysql_query($query))
		{
           $qryUserName = " SELECT id, user_name FROM users as u, photo as p WHERE p.id = '".$body->photoId."' AND u.id=p.user_id AND deleted = 0 ";
           ...
           $deviceTokens = getDeviceTokens($body->userId);
        
           //Send notification if device tokens available
           if(!empty($deviceTokens) && $deviceTokens != '' ){
            $apnNotification->sendNotification($deviceTokens,$message,$action_loc_key,$badge,$sound,$match,$from);
           }
            	...
            	if(mysql_num_rows($res = mysql_query($qry_user)) > 0)
	           	{
                       ...
                       if(mysql_query($qry))
		               {
		                  $lastLikeId = mysql_insert_id();
                           ...
                           // Get device tokens from db
                           $deviceTokens = getDeviceTokens($body->Uid);
