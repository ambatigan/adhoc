...
class photo_flags {
    ...
    public function select($pagging, $condition='', $sort_by='id', $sort_order = 'DESC') {
        ...
        $query = "SELECT p.* FROM " . TBL_PHOTO . " p " . $condition . " AND p.flag ='active' AND p.deleted = '0' ORDER BY  " . $sort_by . " " . $sort_order;
    ...
    public function search_username($pagging, $condition, $sort_by='id', $sort_order = 'DESC') {
        ...
        $query = "SELECT p.id,p.image,p.user_id,p.name,p.tag,u.user_name,p.deleted FROM " . TBL_PHOTO . " p LEFT JOIN ".TBL_USERS." u ON p.user_id = u.id " . $condition . " AND p.flag = 'active' AND p.deleted = '0' ORDER BY  " . $sort_by . " " . $sort_order;
        $userList = $this->db->SimpleQuery($query, "", $pagging, false);
