...
class photo {
    ...
    public function select($pagging, $condition='', $sort_by='id', $sort_order = 'DESC') {
        ...
        $query = "SELECT p.* FROM " . TBL_PHOTO . " p " . $condition . "AND p.flag = 'inactive' ORDER BY  " . $sort_by . " " . $sort_order;
    ...
    public function saearch_username($pagging, $condition, $sort_by='id', $sort_order = 'DESC') {
        ...
        $query = "SELECT p.id,p.image,p.user_id,p.name,p.tag,u.user_name,p.status FROM " . TBL_PHOTO . " p LEFT JOIN ".TBL_USERS." u ON p.user_id = u.id " . $condition . " AND p.flag = 'inactive' ORDER BY  " . $sort_by . " " . $sort_order;
        $userList = $this->db->SimpleQuery($query, "", $pagging, false);
                        ...
                        if($file['error'] == '0')
                            {
                                ...
                                //$image_name = time().'_'.strtolower(str_replace(" ", '', $file['name']));
                                 ...
                                 $image_name = "photo_".$_REQUEST['id'].".jpg";
				$path= PHOTO_IMAGE_PATH.$image_name;

/*echo 'path=='.$path;
echo '<br>';
echo 'temp_path='.$file['tmp_name'];exit;*/
                                ...
                                {
                                        echo $this->error_message = "Error in image upload. Please try again.";
exit;
                                        return false;
                                }
                            }
    ...
    function updateStatus($currStatus,$Id){
        $query = "UPDATE " . TBL_PHOTO . " SET status = '" . $currStatus . "' WHERE id = '".$Id."'";
        $userList = $this->db->SimpleQuery($query, "", $pagging, false);
