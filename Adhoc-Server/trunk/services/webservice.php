...
function subval_sort($a,$subkey)
{
 ...
 //$_POST['json']= '{"name":"GetAllPhotos","body":{"start":"1","total_record":"10","searchText":"","user_id":"362"}}';
//echo json_decode(stripslashes($_GET['json'])).'test';
if(isset($_POST['json']))
{

	//$json_array = json_decode($_POST['json']);
  // $_POST['json']= '{"name":"GetAllPhotos","body":{"start":"1","total_record":"","searchText":"","user_id":"362"}}';
	$json_array = json_decode(stripslashes($_POST['json']));
// $array=array("name"=>"GetAllPhotos","message"=>"Message send to user.","data"=>$get_receiver_birthday);
	...
	if($json_array->name=='GetPromptMsg')
	{
	   	echo GetPromptMsg();
	}
...
function GetPhotoDetail($body)
{
  if($body->photoId == '')
    ...
    else
    {
     ...
     $comment_query = "SELECT c.photo_id,c.comment,c.flag,u.deleted,p.deleted 
                      FROM comments as c 
                      JOIN users as u ON u.id = c.user_id  
                      JOIN photo as p ON p.id = c.photo_id 
                      WHERE c.deleted=0 AND p.deleted = 0 
                            AND c.flag = 'Inactive' 
                            AND u.deleted = 0 
                            AND c.photo_id = '".mysql_real_escape_string($body->photoId)."' 
...
function GetUserProfile($body){
  if($body->userid == '')
    ...
    else
    {
       ...
       $qryUser = "SELECT COUNT(p.user_id) AS totalPost, p.id,u.first_name, u.hometown, u.last_name, SUM(no_of_likes) as totalLikes, u.user_name, u.id as userid, u.image, u.deleted
                FROM users as u LEFT JOIN photo as p ON u.id = p.user_id 
                WHERE u.status = 'Active' AND u.status != 'Blocked' AND u.deleted = 0 AND u.id = '".$body->userid."' GROUP BY u.id ";

	$userComments = "SELECT p.id,u.first_name, u.hometown, u.last_name, SUM(no_of_likes) as totalLikes, u.user_name, u.id as userid, u.image, u.deleted
                FROM users as u INNER JOIN photo as p ON u.id = p.user_id INNER JOIN comments AS c ON p.id = c.photo_id 
                WHERE u.status = 'Active' AND u.status != 'Blocked' AND u.deleted = 0 AND u.id = '".$body->userid."' AND p.deleted = 0 GROUP BY c.user_id";
    $res = mysql_query($userComments);
    $numberofcomment = mysql_num_rows($res);
        ...
        if(@mysql_num_rows($res = @mysql_query($qryUser))){
            ...
            $userScore =  1 * ($row['totalPost']) + (10 * $row['totalLikes']) + (25 * $numberofcomment);
            if( $userScore < 10 ){
                
                $UserRanktext = 'Mail Clerk';
                
            }else if( $userScore < 50 ){
                
                $UserRanktext = 'Copy Intern';
                
            }else if( $userScore < 200 ){
                
                $UserRanktext = 'Junior Copywriter';
                
            }else if( $userScore < 400 ){
                
                $UserRanktext = 'Campaign Manager';
                
            }else if( $userScore < 800 ){
                
                $UserRanktext = 'Senior Copywriter';
                
            }else if( $userScore < 1600 ){
                
                $UserRanktext = 'Copy Chief';
                
            }else if( $userScore < 2000 ){
                
                $UserRanktext = 'Creative Director';
                
            }else if( $userScore < 4000 ){
                
                $UserRanktext = 'Partner';
                
            }else if( $userScore < 10000 ){
                
                $UserRanktext = 'Name on the Door';
                
            }else if( $userScore > 10000 ){
                
                $UserRanktext = 'Advertising Grandmaster';
            ...
            $result['data']['UserRank'] = $UserRanktext;
            $iConter ++;
		}
...
function GetPromptMsg()
{
        ...
        $qry = " SELECT id,prompt FROM `prompts` where status='Active'";
        $result = array();
        ...
        if(@mysql_num_rows($res = @mysql_query($qry))){
			while ($row = mysql_fetch_assoc($res))
			{
             ...
             $array = array("status"=>"GET_PROMPT_MSG_1","message"=>"Request Success","data"=>$result);
             return json_encode($array);

         }
         else
		{
			$array = array("status"=>"GET_PROMPT_MSG_2","message"=>"No Records Found");
			return json_encode($array);
		}
